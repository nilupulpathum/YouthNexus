...
<section class="dw-page" aria-label="Club health monitoring">
  ...
  <div class="dw-toolbar" aria-label="Club health tools">
    <div class="dw-toolbar__search yn-search dw-search"><label class="visually-hidden" for="health-search">Search clubs</label><span class="yn-search__icon dw-search__icon" aria-hidden="true"><?= yn_icon('search') ?></span><input id="health-search" type="search" placeholder="Search by club name, code, or division" data-health-search></div>
    <button class="yn-btn yn-btn--secondary dw-button dw-button--secondary yn-filter-toggle" type="button" data-filter-toggle aria-controls="health-filters" aria-expanded="false"><?= yn_icon('filter') ?> Filters</button>
  </div>
  ...
  <section class="dw-filter-panel" id="health-filters" hidden>
    ...
    <div class="dw-filter-actions"><button class="yn-btn yn-btn--secondary dw-button dw-button--secondary yn-filter-clear" type="button" data-health-reset>Clear filters</button><button class="yn-btn yn-btn--primary dw-button dw-button--primary yn-filter-apply" type="button" data-health-apply>Apply filters</button></div>
  </section>
  ...
  <section aria-labelledby="club-health-list-title">
    ...
    <div class="dch-grid" data-health-grid>
      <?php foreach ($clubs as $club): ?>
        ...
        <article class="dch-card dch-card--<?= $e($tone) ?>" data-health-card data-search="<?= $e($search) ?>" data-status="<?= $e(strtolower($score['health_status'])) ?>" data-score="<?= $e($score['overall_score']) ?>" data-flags="<?= (int) $club->open_flags ?>" data-name="<?= $e(strtolower($club->club_name)) ?>">
          ...
          <?php $pillLabel = $label; $pillTone = strtolower($score['health_status']); require __DIR__ . '/../partials/divisional/status-pill.view.php'; unset($pillLabel, $pillTone); ?>
    ...
    <?php $emptyTitle = 'No clubs found'; $emptyMessage = 'No clubs match the current division and filters.'; $emptyVisible = !$clubs; $emptyIcon = 'file'; $emptyAttribute = 'data-health-empty'; require __DIR__ . '/../partials/empty-state.view.php'; ?>
  </section>
</section>
