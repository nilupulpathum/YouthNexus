...
<div class="fund-transfer-module">
    ...
    <div class="ft-header-bar">
        <div class="ft-header-titles">
            <h2 class="ft-section-title">National Fund Disbursement &amp; Zonal Ledger</h2>
            <p class="ft-section-desc">Manage inter-governmental grants, RTGS clearance, and zonal treasury allocations.</p>
        </div>
        <div class="ft-header-actions">
            <a href="<?= ROOT ?>/fundtransfer/exportledger?<?= http_build_query($filters) ?>" class="ft-btn ft-btn-outline" id="btnDownloadLedger" title="Export Ledger to CSV">
                <span class="ft-btn-icon">&#8681;</span> Download Ledger
            </a>
    ...
    <div class="ft-cards-row">
        ...
        <div class="ft-stat-card">
            ...
            <div class="ft-stat-sub">Total across <?= count($zones) ?> Zonal Offices</div>
        </div>
        ...
        <div class="ft-stat-card ft-stat-card-budget">
            <div class="ft-stat-head">
                <span class="ft-stat-title">ANNUAL BUDGET <?= date('Y') ?></span>
                <span class="ft-badge ft-badge-blue"><?= $utilizedPct ?>% Utilized</span>
            </div>
            <div class="ft-stat-big"><?= $fmtLKR($budgetCap) ?></div>
            <div class="ft-stat-sub">Cap: LKR <?= number_format($budgetCap / 1000000, 1) ?>M &nbsp;&bull;&nbsp; Rem: LKR <?= number_format($remBudget / 1000000, 1) ?>M</div>
    ...
    <!-- Filter Bar -->
    <form method="GET" action="<?= ROOT ?>/fundtransfer" id="ftFilterForm" class="ft-filter-bar">
        <div class="ft-search-box-wrap">
            ...
            <input type="text" name="search" class="ft-filter-input" placeholder="Search by Zone name, Reference, or Cheque No..." value="<?= $activeSearch ?>">
        </div>
        ...
        <select name="quarter" class="ft-filter-select" id="ftFilterQuarter">
            <option value="">Current Quarter (<?= htmlspecialchars($qLabel) ?>)</option>
        ...
        <?php if (!empty($activeSearch) || $activeZone > 0 || $activeStatus !== 'All' || !empty($activeQuarter)): ?>
            <a href="<?= ROOT ?>/fundtransfer" class="ft-btn ft-btn-sm ft-btn-clear">Reset</a>
        <?php endif; ?>
    </form>
    ...
    <div class="ft-table-card">
        ...
        <div class="ft-table-footer">
            <div class="ft-footer-count">
                Showing <strong><?= count($transfers) ?></strong> recorded zonal transfer<?= count($transfers) !== 1 ? 's' : '' ?> in FY <?= date('Y') ?>
            </div>
            <div class="ft-pages">
    ...
    <div class="ft-popup" id="fundAllocationPopup">
        ...
        <div class="ft-popup-header">
            <div class="ft-popup-title">
                <div class="ft-header-icon">&#127974;</div>
                <div>
                    <h1 id="allocModalTitle">New Fund Allocation</h1>
                    <p>Transfer funds to zonal office ledger</p>
                </div>
            </div>
        ...
        <!-- Popup Form Body -->
        <form method="POST" action="<?= ROOT ?>/fundtransfer/create" id="fundAllocationForm" class="ft-popup-body">
            ...
            <!-- Target Zonal Office -->
            <div class="ft-field">
                <label for="alloc_zone_id">Target Zonal Office <span class="ft-required">*</span></label>
                <select class="ft-input" name="zone_id" id="alloc_zone_id" required>
                    <option value="">— Select Target Zonal Office —</option>
                    <?php foreach ($zones as $z): ?>
            ...
            <div class="ft-bank-box">
                ...
                <span class="ft-verified-badge">&#10003; Verified Core Account</span>
            </div>
