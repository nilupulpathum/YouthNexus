<?php
/**
 * Zone Attendance — shell placeholder.
 * Full attendance statistics across the zone land in Z4.
 */
$escape = static function ($value) {
    return htmlspecialchars((string) $value, ENT_QUOTES, 'UTF-8');
};

require __DIR__ . '/../partials/icons.view.php';
require __DIR__ . '/../layouts/dashboard-start.view.php';
?>

<section class="club-page" aria-labelledby="zonal-attendance-heading">
    <h1 id="zonal-attendance-heading" class="sr-only">Zone attendance</h1>

    <p class="club-back-row"><a class="club-btn-secondary" href="<?= ROOT ?>/zonalsecretary"><span aria-hidden="true">‹</span> Secretary Overview</a></p>

    <section class="club-panel" aria-labelledby="zonal-attendance-panel-heading">
        <div class="club-panel-header">
            <div>
                <p class="club-eyebrow">Gampaha Zone</p>
                <h2 id="zonal-attendance-panel-heading">Zone attendance</h2>
            </div>
        </div>

    <p class="club-muted">Attendance statistics aggregated across divisions will be available here.</p>
    </section>
</section>

<link rel="stylesheet" href="<?= ROOT ?>/assets/css/club.css">

<?php require __DIR__ . '/../layouts/dashboard-end.view.php'; ?>
