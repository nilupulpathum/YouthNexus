...
<section class="dw-page dtd-page" aria-label="Divisional Coordinator dashboard">
  ...
  <div class="dtd-grid">
    <section class="dw-panel dtd-panel" aria-labelledby="coordinator-applications-title">
      <header class="dw-panel__header"><div><h2 id="coordinator-applications-title">Pending Club Registrations</h2><p>Applications awaiting your decision</p></div><a class="dw-button dw-button--ghost db-view-button" href="<?= ROOT ?>/clubregistrationapproval/index">Review All</a></header>
      <?php if ($applications): ?>
        <div class="dw-table-wrap"><table class="dw-table"><thead><tr><th>Club</th><th>Proposer</th><th>Category</th><th>Members</th><th>Submitted</th></tr></thead><tbody>
      ...
      <?php else: ?><div class="dtd-empty"><span><?= yn_icon('check') ?></span><strong>No pending registrations</strong><p>New club applications will appear here.</p></div><?php endif; ?>
    </section>

    <section class="dw-panel dtd-panel" aria-labelledby="coordinator-events-title">
      <header class="dw-panel__header"><div><h2 id="coordinator-events-title">Pending Event Approvals</h2><p>Division and club events requiring a decision</p></div><a class="dw-button dw-button--ghost db-view-button" href="<?= ROOT ?>/eventapproval">Review All</a></header>
      ...
      </div><?php else: ?><div class="dtd-empty"><span><?= yn_icon('check') ?></span><strong>No pending event approvals</strong><p>Submitted events will appear here.</p></div><?php endif; ?>
    </section>

    <section class="dw-panel dtd-panel" aria-labelledby="coordinator-health-title">
      <header class="dw-panel__header"><div><h2 id="coordinator-health-title">Club Health Overview</h2><p>Lowest current health scores are shown first</p></div><a class="dw-button dw-button--ghost db-view-button" href="<?= ROOT ?>/divisionalclubhealth">View Details</a></header>
      ...
      </div><?php else: ?><div class="dtd-empty"><span><?= yn_icon('info') ?></span><strong>No club health results</strong><p>Run Club Health to calculate current scores.</p></div><?php endif; ?>
    </section>
