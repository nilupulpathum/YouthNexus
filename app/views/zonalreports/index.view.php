...
$pageDescription = 'Generate and review reports for your zone';
...
?>
<section class="dw-page" aria-label="Zonal reports">
  ...
  <div class="dr-library-actions">
    <?php if ($userRole === 'ZonalSecretary' && $aggregateTypeAvailable): ?>
      <a class="dw-button dw-button--secondary db-secondary-action" href="<?= ROOT ?>/zonalreports/create?mode=aggregate">Aggregate Reports</a>
    <?php endif; ?>
    <a class="dw-button dw-button--primary db-primary-action" href="<?= ROOT ?>/zonalreports/create">Generate Report</a>
  </div>
    ...
    <div class="dr-report-grid" data-report-body>
      <?php foreach ($reports as $report): ?>
        <?php
          ...
          $generatedBy = trim((string) $report->generated_by_name) ?: 'Zonal officer';
          $archivedBy = trim((string) ($report->archived_by_name ?? '')) ?: 'Zonal officer';
        ?>
        <article class="dr-report-card<?= $report->status === 'Archived' ? ' dr-report-card--archived' : '' ?>" data-report-row data-search="<?= $e($search) ?>" data-category="<?= $e(strtolower($report->category)) ?>" data-format="<?= $e(strtolower($report->format)) ?>" data-status="<?= $e(strtolower($report->status)) ?>" data-date="<?= $e(substr($report->generated_at, 0, 10)) ?>" data-type="<?= $e(strtolower($report->type_name)) ?>">
          ...
          <p class="dr-report-card__scope">Zonal level</p>
          ...
          <footer class="dr-report-card__footer">
            ...
            <div class="dr-report-card__actions">
              <?php if ($report->format === 'PDF'): ?><a class="dw-button dw-button--ghost" href="<?= ROOT ?>/zonalreports/pdf/<?= (int) $report->report_id ?>" aria-label="Download <?= $e($report->type_name) ?> as PDF"><?= yn_icon('download') ?> PDF</a><?php elseif ($report->format === 'CSV'): ?><a class="dw-button dw-button--ghost" href="<?= ROOT ?>/zonalreports/export/<?= (int) $report->report_id ?>" aria-label="Download <?= $e($report->type_name) ?> as CSV"><?= yn_icon('download') ?> CSV</a><?php endif; ?>
              <a class="dw-button dw-button--secondary db-view-button" href="<?= ROOT ?>/zonalreports/preview/<?= (int) $report->report_id ?>">View Details</a>
            </div>
          </footer>
        </article>
      <?php endforeach; ?>
