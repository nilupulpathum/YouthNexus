...
$pageDescription = 'Review calculated club performance and concerns within your zone';
$currentRoute = 'zonalclubhealth';
...
$flagLabels = [
    'ZonalTreasurer' => 'Raise Financial Concern',
    'ZonalSecretary' => 'Raise Event or Attendance Concern',
    'ZonalCoordinator' => 'Raise Concern',
];
$flagCategoryLabels = [
...
<section class="dw-page" aria-label="Club health monitoring">
  ...
  <div class="dw-toolbar" aria-label="Club health tools">
    ...
    <a class="dw-button dw-button--secondary" href="<?= ROOT ?>/zonalclubhealth/export"><?= yn_icon('download') ?> Export</a>
  </div>
  ...
  <section aria-labelledby="club-health-list-title">
    <div class="dw-section-header"><div><h2 id="club-health-list-title">Clubs in <?= $e($zone->zonal_name) ?></h2><p>Open a club to review the records behind its score</p></div><span class="dw-count" data-health-count><?= count($clubs) ?> clubs</span></div>
    ...
    <div class="dw-empty-state<?= !$clubs ? ' is-visible' : '' ?>" data-health-empty><span class="dw-empty-state__icon"><?= yn_icon('file') ?></span><strong>No clubs found</strong><p>No clubs match the current zone and filters.</p></div>
  </section>
</section>
...
<div class="dw-modal" id="club-health-flag" role="dialog" aria-modal="true" aria-hidden="true" hidden>
  ...
  <form class="dw-modal__dialog" action="" method="post" data-health-flag-form data-action-base="<?= ROOT ?>/zonalclubhealth/flag/">
