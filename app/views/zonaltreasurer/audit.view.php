    ...
    <?php foreach ($flags as $flag): ?>
        ...
        <article class="audit-exception" data-flag="<?= $escape($flag['id']) ?>" data-division="<?= $escape($flag['division']) ?>" data-status="<?= $escape($flag['status']) ?>">
            ...
            <div class="audit-head-right">
                <?php if (in_array($flag['status'], ['Open', 'Clarification requested'], true)): ?><button type="button" class="audit-btn-outline" data-modal-open="clarify-modal" data-flag-id="<?= (int) ($flag['flag_id'] ?? 0) ?>" data-division="<?= $escape($flag['division']) ?>">Request clarification</button><?php endif; ?>
                <?php if ($flag['status'] === 'Response received'): ?><button type="button" class="audit-btn-outline" data-modal-open="resolve-modal" data-flag-id="<?= (int) ($flag['flag_id'] ?? 0) ?>">Resolve with note</button><?php endif; ?>
                <?php if (!in_array($flag['status'], ['Resolved', 'Escalated to NYSC'], true)): ?><button type="button" class="audit-btn-outline" data-modal-open="escalate-modal" data-flag-id="<?= (int) ($flag['flag_id'] ?? 0) ?>">Escalate to NYSC</button><?php endif; ?>
            </div>
        </article>
    <?php endforeach; ?>
...
<div class="audit-overlay" id="flag-modal"><div class="audit-modal audit-clarify-modal"><div class="audit-modal-head"><div><h1>Flag discrepancy</h1><p>Only divisions in this zone can be audited here.</p></div><button type="button" class="audit-close-x" data-modal-close aria-label="Close"><?= yn_icon('close') ?></button></div><form method="post" action="<?= ROOT ?>/zonaltreasurer/flag"><input type="hidden" name="csrf_token" value="<?= $escape($csrf_token) ?>"><div class="audit-modal-body"><label class="audit-field-label" for="audit-division">Division</label><select id="audit-division" class="audit-field-box" name="division_id"><?php foreach ($reports as $report): ?><option value="<?= (int) ($report['division_id'] ?? 0) ?>"><?= $escape($report['division']) ?></option><?php endforeach; ?></select><label class="audit-field-label" for="audit-type">Discrepancy type</label><select id="audit-type" class="audit-field-box" name="type"><option value="MissingReceipt">Missing receipt</option><option value="FundHoarding">Fund hoarding</option><option value="HighVoidRate">High void rate</option></select><label class="audit-field-label" for="audit-reference">Report or ledger reference</label><input id="audit-reference" class="audit-field-box" name="reference" required maxlength="100"><label class="audit-field-label" for="audit-amount">Amount (LKR)</label><input id="audit-amount" class="audit-field-box" name="amount" type="number" min="0.01" step="0.01" required><label class="audit-field-label" for="audit-reason">Discrepancy reason</label><textarea id="audit-reason" class="audit-textarea" name="reason" required maxlength="1000"></textarea></div><div class="audit-modal-foot-clarify"><button type="button" class="audit-btn-cancel-border" data-modal-close>Cancel</button><button type="submit" class="audit-btn-send">Create audit flag</button></div></form></div></div>
