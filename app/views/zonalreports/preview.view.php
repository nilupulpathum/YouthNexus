  ...
  <article class="dw-panel dr-document">
    <header class="dr-document__header">
      <div>
        ...
        <div class="dr-document__meta"><span>Scope: <strong>Zonal</strong></span><span>Range: <strong><?= $e(date('d M Y', strtotime($report->date_range_start))) ?> - <?= $e(date('d M Y', strtotime($report->date_range_end))) ?></strong></span><span>Category: <strong><?= $e($report->category) ?></strong></span></div>
      </div>
    ...
    <div class="dr-document__body">
      ...
      <div class="dr-verification-note"><?= yn_icon('info') ?><p><strong>Report snapshot:</strong> The values shown here were recorded when this report was generated for <?= $e($zone->zonal_name) ?>.</p></div>
    ...
    <footer class="dr-document__footer">
      <div class="dr-document__footer-left">
        <a class="dw-button dw-button--ghost db-view-button db-view-button--back" href="<?= ROOT ?>/zonalreports">Back to Reports</a>
        <?php if ($report->status === 'Archived'): ?>
          <form method="post" action="<?= ROOT ?>/zonalreports/restore/<?= (int) $report->report_id ?>" onsubmit="return confirm('Restore this report to the active report list?');"><input type="hidden" name="csrf_token" value="<?= $e($csrfToken) ?>"><button class="dw-button dw-button--primary db-confirm-action" type="submit">Restore Report</button></form>
        <?php else: ?>
          <form method="post" action="<?= ROOT ?>/zonalreports/archive/<?= (int) $report->report_id ?>" onsubmit="return confirm('Archive this report? It will remain available under the Archived filter.');"><input type="hidden" name="csrf_token" value="<?= $e($csrfToken) ?>"><button class="dw-button dw-button--danger" type="submit">Archive Report</button></form>
        <?php endif; ?>
      </div>
      <div class="dr-document__footer-right"><a class="dw-button dw-button--secondary" href="<?= ROOT ?>/zonalreports/export/<?= (int) $report->report_id ?>"><?= yn_icon('download') ?> Download CSV</a><a class="dw-button dw-button--primary" href="<?= ROOT ?>/zonalreports/pdf/<?= (int) $report->report_id ?>"><?= yn_icon('download') ?> Download PDF</a></div>
    </footer>
  </article>
</section>
