...
<section class="dw-page dtd-page" aria-label="Divisional Secretary dashboard">
  ...
  <div class="dtd-grid">
    <section class="dw-panel dtd-panel" aria-labelledby="secretary-events-title">
      ...
      <?php if ($events): ?><div class="dtd-list"><?php foreach ($events as $event): ?><article class="dtd-list__item"><div><strong><?= $e($event->title) ?></strong><span><?= $e($event->organiser ?: 'Division') ?> - <?= $e($event->location ?: 'Location not set') ?></span></div><div class="dtd-list__value"><strong><?= $e($date($event->start_datetime)) ?></strong><span><?= $e($event->event_type ?: 'General') ?></span></div><?php $status = 'Approved'; require __DIR__ . '/../partials/divisional/status-pill.view.php'; ?></article><?php endforeach; ?></div><?php else: ?><?php $emptyTitle = 'No upcoming events'; $emptyMessage = 'Approved future events will appear here.'; $emptyVisible = true; $emptyIcon = 'calendar'; require __DIR__ . '/../partials/empty-state.view.php'; ?><?php endif; ?>
    </section>
    ...
    <section class="dw-panel dtd-panel" aria-labelledby="secretary-health-title">
      ...
      <?php if ($clubHealth): ?><div class="dtd-health-list"><?php foreach ($clubHealth as $club): $score = max(0, min(100, (float) $club->overall_health_score)); ?><article class="dtd-health"><div class="dtd-health__score dtd-health__score--<?= $e($healthClass($club->health_status)) ?>"><strong><?= $e(number_format($score, 0)) ?></strong><span>/100</span></div><div class="dtd-health__body"><div><strong><?= $e($club->club_name) ?></strong><?php if ((int) $club->open_flags > 0): ?><span><?= (int) $club->open_flags ?> open <?= (int) $club->open_flags === 1 ? 'concern' : 'concerns' ?></span><?php endif; ?></div><div class="dtd-health__bar"><span class="dtd-health__fill--<?= $e($healthClass($club->health_status)) ?>" style="width: <?= $e(number_format($score, 2, '.', '')) ?>%"></span></div></div><span class="dtd-health__label dtd-health__label--<?= $e($healthClass($club->health_status)) ?>"><?= $e($healthLabel($club->health_status)) ?></span></article><?php endforeach; ?></div><?php else: ?><?php $emptyTitle = 'No club health results'; $emptyMessage = 'Run Club Health to calculate current scores.'; $emptyVisible = true; $emptyIcon = 'info'; require __DIR__ . '/../partials/empty-state.view.php'; ?><?php endif; ?>
    </section>
