...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const readCard = (card) => { card.classList.remove('is-unread'); card.dataset.unread = 'false'; card.querySelector('[data-action="mark-read"]')?.remove(); updateUnread(); const id = card.dataset.announcementId || ''; if (id) { const form = new FormData(); form.append('announcement_id', id); form.append('format', 'json'); fetch('zonalannouncements/markread', { method: 'POST', body: form, headers: { 'Accept': 'application/json' } }).catch(() => {}); } };
    ...
    let emailChoiceMade = false;
    const publish = (emailed) => { emailChoiceMade = true; if (emailed) { const flag = document.createElement('input'); flag.type = 'hidden'; flag.name = 'emailed'; flag.value = '1'; form.appendChild(flag); } form.submit(); };
    form.addEventListener('submit', (event) => { const title = document.getElementById('zonal-publish-heading').value.trim(); const body = document.getElementById('zonal-publish-body').value.trim(); if (!title || !body) { event.preventDefault(); error.textContent = 'Title and body are required.'; error.hidden = false; return; } if (document.getElementById('zonal-publish-priority').value === 'Urgent' && !emailChoiceMade) { event.preventDefault(); emailModal.hidden = false; return; } });
    emailModal.querySelectorAll('[data-close-email]').forEach((button) => button.addEventListener('click', () => { emailModal.hidden = true; publish(false); }));
    document.getElementById('zonal-email-confirm').addEventListener('click', () => { emailModal.hidden = true; publish(true); });
    }
});
